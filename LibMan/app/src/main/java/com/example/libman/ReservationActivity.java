...
public class ReservationActivity extends AppCompatActivity {
    ...
    private void reserveBook() {
        ...
        if (selectedMemberName.isEmpty()) {
            Toast.makeText(this, "Please select a member name to reserve!", Toast.LENGTH_SHORT).show();
            return;
        }
        // Get book details
        Cursor bookCursor = dbHelper.getBookByName(bookName);
        if (bookCursor != null && bookCursor.moveToFirst()) {
            ...
            if (bookIdColumnIndex != -1 && authorColumnIndex != -1 && publisherColumnIndex != -1 && quantityColumnIndex != -1) {
                ...
                if (quantity > 0) {
                    // Validate that the logged-in user matches the selected member
                    {
                            ...
                            // Reserve the book
                            dbHelper.reserveBookAdmin(bookId, selectedMemberName);
