...
public class DbHelper extends SQLiteOpenHelper {
    ...
    // Table names for user and admin databases
    private static final String USER_TABLE = "user";
    private static final String ADMIN_TABLE = "admin";
    private static final String BOOK_TABLE = "book";
    private static final String TABLE_MEMBER = "members";
    private static final String TABLE_KEY_DETAILS = "key_details";

    // User and admin table columns
    ...
    private static final String  RESERVE_BOOK_NAME = "book_name";
    ...
    public void onCreate(SQLiteDatabase db) {
        // Creating tables for the user database
        ...
        // Creating tables for the admin database
        String createAdminTableQuery = "CREATE TABLE IF NOT EXISTS " + ADMIN_TABLE + "("
                + ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "
                + NAME + " TEXT, "
                + EMAIL + " TEXT, "
                + PASSWORD + " TEXT, "
        ...
        // Creating the book table for both user and admin databases
        ...
        String CREATE_MEMBER_TABLE = "CREATE TABLE IF NOT EXISTS " + TABLE_MEMBER + "("
        ...
        // Creating the key details table
        String createKeyDetailsTableQuery = "CREATE TABLE IF NOT EXISTS " + TABLE_KEY_DETAILS + "("
                + "key_value TEXT PRIMARY KEY"
                + ")";
        db.execSQL(createKeyDetailsTableQuery);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_KEY_DETAILS);
        onCreate(db);
    }
    ...
    public boolean loginAdmin(String email, String password) {
        ...
        try {
            String selectQuery = "SELECT * FROM " + ADMIN_TABLE + " WHERE " + EMAIL + " = ? AND " + PASSWORD + " = ?";
            cursor = db.rawQuery(selectQuery, new String[]{email, password});
            if (cursor != null && cursor.getCount() > 0) {
                isLoggedIn = true;
            }
    ...
    // Register methods for Admin
    public void registerAdmin(String name, String email, String password, String confirmPassword) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(NAME, name);
        values.put(EMAIL, email);
    ...
    // Method to insert a new key into the database
    ...
    public void reserveBookAdmin(int bookId, String memberName) {
        SQLiteDatabase db = getWritableDatabase();
    ...
    public Cursor getReservationByMemberName(String memberName) {
        SQLiteDatabase db = getReadableDatabase();
        String query = "SELECT * FROM " + RESERVE_TABLE + " WHERE " + RESERVE_MEMBER_NAME + " = ?";
    ...
    public void returnBook(String bookName) {
        ...
        db.delete(RESERVE_TABLE,
                RESERVE_MEMBER_NAME + " = (SELECT " + MEMBER_EMAIL + " FROM " + TABLE_MEMBER +
                        " WHERE " + MEMBER_EMAIL + " = ? LIMIT 1)",
                new String[]{bookName});
    ...
    public void returnBooknew(String bookName, int bookId) {

        try (SQLiteDatabase db = this.getWritableDatabase()) {
            // Check if the reservation exists
            Cursor cursor = db.rawQuery("SELECT * FROM " + RESERVE_TABLE + " WHERE " +
                    BOOK_NAME + " = ? AND " + MEMBER_NAME + " = ?", new String[]{bookName, String.valueOf(bookId)});

            if (cursor != null && cursor.moveToFirst()) {
                // Reservation exists for this book and member, proceed to delete it
                db.delete(RESERVE_TABLE, RESERVE_BOOK_NAME + " = (SELECT " + BOOK_ID + MEMBER_NAME+ " FROM " + BOOK_TABLE +
                        " WHERE " + BOOK_NAME + " = ? AND " + MEMBER_NAME+ " = ? "  , new String[]{bookName});
                // Increase the book quantity
                db.execSQL("UPDATE " + BOOK_TABLE + " SET " + BOOK_QUANTITY + " = " + BOOK_QUANTITY + " + 1 WHERE " + BOOK_NAME + " = ?",
                        new String[]{bookName});
            }
            ...
            Log.e("DbHelper", "Error occurred while returning book", e);
        }
    }
